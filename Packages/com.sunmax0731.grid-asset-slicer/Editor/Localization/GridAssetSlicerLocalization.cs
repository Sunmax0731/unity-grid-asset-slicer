...
namespace Sunmax.GridAssetSlicer.Editor.Localization
{
    public static class GridAssetSlicerLocalization
    {
        private static readonly IReadOnlyDictionary<string, string> Japanese = new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            ["detachedPreviewReconnect"] = "Tools > Grid Asset Slicer > メイン画面 を開き直して、プレビューを再接続してください。",
            ["detachedHelpReconnect"] = "Tools > Grid Asset Slicer > メイン画面 を開き直して、ヘルプを再接続してください。",
            ["export"] = "書き出し...",
            ["saveSession"] = "セッション保存",
            ["loadSession"] = "セッション読込",
            ["gridSettings"] = "グリッド設定",
            ...
            ["cellWidth"] = "セル幅",
            ["cellHeight"] = "セル高さ",
            ["marginController"] = "余白コントローラ",
            ["marginControllerHelp"] = "この元画像コントローラ上のガイド線をドラッグすると、プレビューを開かずに余白を調整できます。",
            ["marginControllerNoSource"] = "余白コントローラを使うには元画像を選択してください。",
            ["marginControllerValues"] = "左 {0} / 上 {1} / 右 {2} / 下 {3}",
            ["variableGrid"] = "可変グリッド境界",
            ["variableGridHelp"] = "各列幅または各行高を個別に調整したい場合に有効化します。",
            ["customColumns"] = "列ごとに個別設定",
            ["customRows"] = "行ごとに個別設定",
            ["columnBoundary"] = "列区切り位置",
            ["rowBoundary"] = "行区切り位置",
            ["columnWidth"] = "列幅",
            ["rowHeight"] = "行高",
            ["output"] = "出力",
            ...
            ["workspace"] = "ワークスペース",
            ["workspaceEmptyTitle"] = "元画像が選択されていません。",
            ["workspaceEmptyBody"] = "ツールバーから Texture2D を選択し、左の設定パネルで行数、列数、余白を調整してください。",
            ["workspaceFlowHint"] = "ワークフロー: 元画像 -> 設定 -> プレビュー -> 書き出し -> セッション保存",
            ["workspaceSummary"] = "現在の概要",
            ["workspaceActions"] = "次のアクション",
            ["workspaceWarnings"] = "現在の警告",
            ["workspaceSelectedCell"] = "選択セル: 行 {0}, 列 {1}",
            ["workspaceNoSelectedCell"] = "セルは未選択です。詳細を表示するにはプレビューウィンドウで選択してください。",
            ...
            ["help.outputWidth"] = "書き出し時に各セルを統一する横幅です。OFF の場合は切り出した元サイズを使います。",
            ["help.outputHeight"] = "書き出し時に各セルを統一する高さです。OFF の場合は切り出した元サイズを使います。",
            ...
            ["error.availableWidthPerCell"] = "利用可能なグリッド幅では各セルに 1 ピクセル以上を割り当てられません。",
            ["error.availableHeightPerCell"] = "利用可能なグリッド高さでは各セルに 1 ピクセル以上を割り当てられません。",
            ["error.outputFolderRequired"] = "出力フォルダが必要です。",
            ["error.outputPrefixRequired"] = "出力接頭辞が必要です。",
            ["error.startIndexNonNegative"] = "開始番号は 0 以上である必要があります。",
            ["error.numberPaddingNonNegative"] = "連番桁数は 0 以上である必要があります。",
            ["error.outputResizeBothRequired"] = "出力幅と出力高さは両方を同時に指定してください。",
            ["error.outputWidthPositive"] = "出力幅を指定する場合は 1 以上である必要があります。",
            ["error.outputHeightPositive"] = "出力高さを指定する場合は 1 以上である必要があります。",
