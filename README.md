...
- Version: 0.1.1
- Current implementation state: Grid slicing / session replay / PNG export / optional export resize / detached preview / large margin controller / variable row-column boundaries / foldout-based single settings pane / workspace empty-state and summary flow
...
4. 大きめの `Margin Controller` で余白ガイドをドラッグして調整する。
5. 数値入力またはスライダーで grid 値を微調整する。
6. 必要に応じて各列・各行の区切り位置を個別に調整する。
7. implicit cell size が割り切れない場合でも、そのまま preview / export する。
8. 切り出し予定セルを preview する。
9. セル単位で採用・除外を切り替える。
10. 選択セルを個別 PNG として出力する。
11. slice session JSON を保存する。
12. JSON を読み込み、同条件で再実行できる。
