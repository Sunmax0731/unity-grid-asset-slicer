...
```json
{
  ...
  "grid": {
    ...
    "cellWidth": 128,
    "cellHeight": 128,
    "columnWidths": [120, 136, 128, 128, 128, 128, 128, 128],
    "rowHeights": [128, 128, 128, 128, 128, 128, 128, 128]
  },
  "selection": {
    "excludedCells": [
      { "row": 0, "column": 7 },
...
- `grid` の数値は pixel 単位。
- `grid.columnWidths` / `grid.rowHeights` は optional。指定する場合は `columns` / `rows` と同じ要素数を持ち、各値は 1 以上である必要がある。
- `selection.excludedCells` は 0-based row / column。
- default は include とし、除外 cell だけを保存する。
- `export.outputWidth` / `export.outputHeight` は optional。使う場合は両方を同時に指定し、1 以上の整数にする。
